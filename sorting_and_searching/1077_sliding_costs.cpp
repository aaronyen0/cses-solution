...
void solve() {
    ...
    for(int i = 1; i <= n; ++i) {
        ...
        bit.update(idx, 1);
        if(i >= k) {
            ll median = idx2val[bit.find_kth(k2)];
            if(i == k) {
                for(int i = 1; i <= k; ++i) total += abs(median - vals[i]);
            } else {
                total -= abs(vals[i - k] - pre_median);
                total += abs(vals[i] - pre_median);
                if (median > pre_median) {
